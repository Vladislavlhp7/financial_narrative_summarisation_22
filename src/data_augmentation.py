...
def perform_translation_single_pass_fr(
    sents, model, tokenizer, device: str = "cpu", lang="fr"
):
    """
    ...
    txt_translated_encoded = model.generate(
        **tokenizer(
            txt_formatted,
            return_tensors="pt",
            padding=True,
    ...
    txt_translated_decoded = [
        tokenizer.decode(t, skip_special_tokens=True) for t in txt_translated_encoded
    ]
...
def perform_backtranslation_fr(
    sents,
    lang_original,
    lang_tmp,
    file_path,
    ...
    if save and os.path.exists(file_path):
        os.remove(file_path)
    for i in tqdm(
        range(0, len(sents), batch_size),
        desc=f"Back-translating {lang_original}-{lang_tmp}",
    ):
        batch_sents = sents[i : i + batch_size]
        batch_pass1 = perform_translation_single_pass_fr(
            batch_sents, model1, model1_tkn, device=device, lang=lang_tmp
        )
        batch_pass2 = perform_translation_single_pass_fr(
            batch_pass1, model2, model2_tkn, device=device, lang=lang_original
        )
...
def backtranslate(
    ...
    if lang_tmp == "fr":
        new_sents = perform_backtranslation_fr(
            sents,
            lang_original,
            lang_tmp,
            save=save,
...
def main():
    for downsample_rate in [0.75]:
        for random_state in [42, 43]:
            ...
            df = pd.read_csv(
                "../tmp/train_downsample_{0}_random_{1}.csv".format(
                    downsample_rate, random_state
                )
            )
            sents = df.loc[df.label == 1].sent.tolist()
            new_sents = backtranslate(
                sents,
                lang_original,
                lang_tmp,
                device,
