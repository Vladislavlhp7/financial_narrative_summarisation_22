...
def pad_batch(batch_sent_arr):
    ...
    for train_sents in batch_sent_arr:
        padded_train_sents = torch.zeros(
            max_len, train_sents.shape[1], dtype=torch.float32
        )
...
def batch_str_to_batch_tensors(sentence_list, embedding_model, seq_len: int = 100):
    ...
    batch_sent_arr = [
        get_sentence_tensor(
            embedding_model=embedding_model, sentence=str(sent), seq_len=seq_len
        )
...
class FinRNN(nn.Module):
    ...
    def __init__(
        ...
        if rnn_type == "lstm":
            ...
            raise ValueError(
                "Wrong `rnn_type` variable. Must be either `lstm` or `gru`."
            )
        self.rnn = rnn(
        ...
        self.fully_connected2 = nn.Linear(
            in_features=self.D * hidden_size, out_features=label_size
        )

    def forward(self, sent):
        out = F.tanh(self.fully_connected(sent))
        ...
        logits = self.fully_connected2(linear_combination)
        logits = F.softmax(logits, dim=1)
        return logits, energy


def retrieve_augmented_data_df(
    lang_original: str = "en",
    lang_tmp: str = "fr",
    downsample_rate: float = 0.75,
    random_state: int = 42,
):
...
class FNS2021(Dataset):
    def __init__(
        ...
        if type_ == "testing":
            ...
            if type_load_directly:
                ...
                tr_filename = "../tmp/train_downsample_{0}_random_{1}.csv".format(
                    downsample_rate, random_state
                )
                valid_filename = (
                    "../tmp/validation_downsample_{0}_random_{1}.csv".format(
                        downsample_rate, random_state
                    )
                )
    ...
    def downsample(df: pd.DataFrame, rate: float = 0.5, random_state: int = 1):
        ...
        non_summary_df = resample(
            non_summary_df,
            replace=True,
            n_samples=int(len(non_summary_df) * (1 - rate)),
            random_state=random_state,
        )
        df = pd.concat([summary_df, non_summary_df]).sort_values(
            ["sent_index"]
...
def train_one_epoch(
    ...
    for i, (data, labels) in enumerate(train_dataloader):
        batch_sent_tensor = batch_str_to_batch_tensors(
            sentence_list=data, embedding_model=embedding_model, seq_len=seq_len
        ).to(device)
        ...
        winners = predicted.argmax(
            dim=1
        )  # each sentence has p0 and p1 probabilities with p0 + p1 = 1
        corrects = (
            winners == target
        )  # match predicted output labels with observed labels
        ...
        if i % 1000 == 999:
            ...
            print(
                "  batch {} loss: {} total accuracy: {} summary accuracy: {}, ".format(
                    i + 1, last_loss, last_acc, last_acc_1
                )
            )
            # Log training details on W&B
            metrics = binary_classification_metrics(
                true_labels=true_labels, pred_labels=pred_labels, prefix="train:"
            )
...
def validate(
    model, embedding_model, validation_dataloader, criterion, seq_len, device, epoch
):
    running_vloss = 0.0
    running_acc = 0.0  # Total accuracy for both classes
    running_acc_1 = 0.0  # Accuracy for summary class
    ...
    with torch.no_grad():
        for i, (v_data, v_labels) in enumerate(validation_dataloader):
            batch_sent_tensor = batch_str_to_batch_tensors(
                sentence_list=v_data, embedding_model=embedding_model, seq_len=seq_len
            ).to(device)
            ...
            if i % 1000 == 999:
                ...
                last_acc_1 = running_acc_1 / 1000  # summary sent accuracy per batch
                print(
                    "Validation loss {} total accuracy: {} summary accuracy: {}".format(
                        last_loss, last_acc, last_acc_1
                    )
                )
                # Log training details on W&B
                metrics = binary_classification_metrics(
                    true_labels=true_labels, pred_labels=pred_labels, prefix="val:"
                )
...
def run_experiment(config=None, root: str = ".."):
    ...
    with wandb.init(
        resume=False,
        project="extractive_summarisation-data-augmentation-unified",
        config=config,
        entity="yotkovv",
    ):
        config = wandb.config
        ...
        training_data = FNS2021(
            file=f"{root}/tmp/{data_filename}",
            type_="training",
        ...
        train_dataloader = DataLoader(
            training_data, batch_size=config.batch_size, drop_last=True
        )
        print("Loading Validation Data")
        validation_data = FNS2021(
            file=f"{root}/tmp/{data_filename}",
            type_="validation",
            random_state=config.seed,
            downsample_rate=config.downsample_rate,
        ...
        validation_dataloader = DataLoader(
            validation_data, batch_size=config.batch_size, drop_last=True
        )
        print("Loading Testing Data")
        data_filename_test = "validation_corpus_2023-02-07 16-33.csv"  # real validation data is used as test
        test_data = FNS2021(
            file=f"{root}/tmp/{data_filename_test}",
            type_="testing",
            random_state=config.seed,
        )  # use all validation data
        empirical_test_report_size = 2_048
        test_dataloader = DataLoader(
            test_data, batch_size=empirical_test_report_size, drop_last=True
        )
