...
We also discuss the quality of the produced summaries (Section~\ref{subsec:qualitative-discussion}), while claiming that
\begin{enumerate*}
    \item our summary generation process is not effective for our GRU-based system, and
    \item the evaluation metric is problematic because it does not capture semantic importance
\end{enumerate*}.
\\
...
\begin{itemize}
    \item \emph{Use all summaries during sentence extraction} -
    \item \emph{Integrate FinText word embeddings} -
    \item \emph{Perform back-translation as data augmentation} -
    \item \emph{Fine-tune FinBERT (\cite{yang2020finbert})} -
\end{itemize}
...
\begin{itemize}
    \item \emph{Summary Generation} - In our work we take top $k$ sentences based on the model's output probability distribution (Section~\ref{sec:evaluation}).
        ...
        As for the third point, we believe this is one of the main reasons why our GRU architecture returns a summary
        without any whole--sentence overlap with the gold standard (Fig.~\ref{fig:rnn_summary}).
